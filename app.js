...
app.get('/manage-records', function (req, res) {
    const pageId = req.query.pageId;
    const pagenumber = (pageId-1)*10;
    const tempData = data.slice(pagenumber,pageId*10);

    const result={
        Ids: tempData.map(item=>item.id),
        Open:tempData.filter(item=>item.disposition === 'open'),
        ClosedCount:tempData.filter(item=>item.disposition === 'close').length,
